...
public class GeneticAlgorithm {
	...
	public Population cycle(Population pop, String selection, String croisement, String mutation, String remplacement) {
		...
		switch (croisement) {
			case "croisementSimple":
				popIssue = croisementSimplePopulation(pop, selection, mutation, remplacement);
				break;
			case "croisementMonoPoint":
				popIssue = croisementMonoPoint(pop, selection, mutation, remplacement);
				break;
			case "croisementUniforme":
				popIssue = croisementUniformePopulation(pop, selection, mutation, remplacement);
				break;
		}
		return popIssue;
	}
	 ...
	 */
	public Population croisementSimplePopulation(Population pop, String selection, String operateurMutation,
			String remplacement) {
		...
		for (int i = 0; i < pop.getIndividus().length; i++) {
			switch (selection) {
				case "selection2Hasard":
					popSelectionnee = selection2Hasard(pop);
					break;
				case "selection2Meilleur":
					popSelectionnee = selection2Meilleur(pop);
					break;
				case "selection2MeilleurSur5":
					popSelectionnee = selection2MeilleurSur5(pop);
					break;
			}
			...
			switch (remplacement) {
				case "remplacement1Mauvais":
					popIssue = remplacement1Mauvais(popIssue, croisementSimpleIndividu(individu1, individu2));
					break;
			}
		}
	 ...
	 * @return pop croisée
	 */
	public Population croisementMonoPoint(Population pop, String selection, String operateurMutation,
			String remplacement) {
		...
		for (int i = 0; i < pop.getIndividus().length; i++) {
			switch (selection) {
				case "selection2Hasard":
					parentsSelectionnes = selection2Hasard(pop);
					break;
				case "selection2Meilleur":
					parentsSelectionnes = selection2Meilleur(pop);
					break;
				case "selection2MeilleurSur5":
					parentsSelectionnes = selection2MeilleurSur5(pop);
					break;
			}
			...
			switch (remplacement) {
				case "remplacement2Mauvais":
					popIssue = remplacement2Mauvais(popIssue,
							croisementMonoPointParent(parent1, parent2).getIndividus()[0],
							croisementMonoPointParent(parent1, parent2).getIndividus()[1]);
					break;
				case "remplacement2Meilleur":
					popIssue = remplacement2Meilleur(popIssue,
							croisementMonoPointParent(parent1, parent2).getIndividus()[0],
							croisementMonoPointParent(parent1, parent2).getIndividus()[1]);
					break;
			}
	 ...
	 * @return pop croisée
	 */
	public Population croisementUniformePopulation(Population pop, String selection, String operateurMutation,
			String remplacement) {
		...
		for (int i = 0; i < pop.getIndividus().length; i++) {
			switch (selection) {
				case "selection1Hasard":
					individu1 = selection1Hasard(pop);
					break;
				case "selection1Meilleur":
					individu1 = selection1Meilleur(pop);
					break;
			}
			...
			switch (remplacement) {
				case "remplacement1Mauvais":
					popIssue = remplacement1Mauvais(popIssue, croisementUniformeIndividu(individu1, individu2));
					break;
			}
		}
		mutationPopulation(popIssue, operateurMutation);
	...
	/**
	 * mutation bit-flip (swap Mutation) : selectionner 2 genes aleatoirement et les
	 * muter
	 * 
	 * @param individu
	 * @return individu
	...
	private Individu mutationBitFlipIndividu(Individu individu) {
		...
		if (Math.random() < proba_mutation) {
			if (individu.getGenes()[gene1] == 1)
				individu.getGenes()[gene1] = 0;
			else
				individu.getGenes()[gene1] = 1;

			if (individu.getGenes()[gene2] == 1)
				individu.getGenes()[gene2] = 0;
			else
				individu.getGenes()[gene2] = 1;
		}
		return individu;
	}

	/**
	 * mutation k-flip (inversion Mutation) : selectionner aleatoirement k genes et
	 * les muter
	 * 
	 * @param individu
	 * @param k
	...
	private Individu mutationKFlipIndividu(Individu individu, int k) {
		if (k <= individu.getGenes().length) {
			...
			if (Math.random() < proba_mutation) {
				for (int i = 0; i < PositionsGenesSelectiones.length; i++) {
					if (individu.getGenes()[PositionsGenesSelectiones[i]] == 1)
						individu.getGenes()[PositionsGenesSelectiones[i]] = 0;
					else
						individu.getGenes()[PositionsGenesSelectiones[i]] = 1;
				}
			}
		} else {
	...
	private Population remplacement2Mauvais(Population pop, Individu enfant1, Individu enfant2) {
		...
		if (popIssue.getIndividus().length > 0) {
			...
			if (popIssue.getIndividus().length > 1) {
				...
				System.out.println(
						"Cette population contient un seul individu. On ne peut pas remplacer les 2 derniers !");
			}
		}
		return popIssue;
	}
	...
	private Population remplacement2Meilleur(Population pop, Individu enfant1, Individu enfant2) {
		...
		if (popIssuse.getIndividus().length > 0) {
			...
			if (popIssuse.getIndividus().length > 1) {
				...
				System.out.println(
						"Cette population contient un seul individu. On ne peut pas remplacer les 2 meilleurs !");
			}
		}
		return popIssuse;
	}
