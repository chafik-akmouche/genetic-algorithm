...
public class Display {
	 ...
	 * @param fitnessMax
	 */
	public static void affichageFitness(int index, ArrayList<Double> fitnessMin, ArrayList<Double> fitnessMoy,
			ArrayList<Double> fitnessMax) {
	...
	public static void affichageEtatOp(ArrayList<Operator> listeOp) {
		...
		for (int i = 0; i < listeOp.size(); i++) {
			System.out.print("op " + i + ": " + listeOp.get(i).getName() + " | proba/val=" + listeOp.get(i).getProba()
					+ " | nb_fois=" + listeOp.get(i).getNb_fois());
			System.out.print("\n\tReward: [ ");
